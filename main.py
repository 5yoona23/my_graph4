...
valid_audi = df.dropna(subset=["total_audi"])

if len(valid_audi) > 0:

    max_audi_row = valid_audi.loc[
        valid_audi["total_audi"].idxmax()
    ]
    ...
    cut_result = pd.cut(
        valid_audi["total_audi"],
        bins=20,
        include_lowest=True
    )

    counts = cut_result.value_counts(
        sort=False
    )
...
st.info(
    "국가별로 어떤 장르의 영화가 많이 만들어졌는지와 "
    "장르별 영화 편수의 차이를 한눈에 비교할 수 있습니다."
)
